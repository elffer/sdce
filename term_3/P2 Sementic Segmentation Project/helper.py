...
def gen_batch_function(data_folder, image_shape, augmentation=True):
    """
    Generate function to create batches of training data
    :param data_folder: Path to folder that contains all the datasets
    :param image_shape: Tuple - Shape of image
    :param augmentation: True or False - Whether use augmentation or not
    :return:
    """
...
def addNoise(image, amt=0.1):
    new_image = image + amt * image.std() * np.random.random(image.shape)
    new_image = ndimage.gaussian_filter(image, sigma=0.1)
    return new_image


def blur(image):
    new_image = ndimage.gaussian_filter(image, sigma=0.1) # Blur
    return new_image
...
def addBlotch(image, max_dims=[0.2,0.2]):
    ...
    size = np.random.randint(low=2, high=4) #size of each side of box
...
def augment(images, labels=None, amplify=2):
    ...
    if labels is not None:
        ...
        new_labels = np.zeros(((amplify*labels.shape[0]), labels.shape[1], labels.shape[2], labels.shape[3]))
    for i in range(len(images)):
        ...
        for j in range(1, amplify):
            add_r = ( j * shape[0] )
            which_op = np.random.randint(low=0, high=len(ops))
