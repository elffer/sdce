...
// set up index for positions of states and controls in a vector for CppAD
size_t x_start = 0;
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // ***************** Part I: Define the Objective (Cost) Expresion ***************//
    // The first element of fg represents the cost
    fg[0] = 0;

    // Add up the state cost
    ...
    for(size_t i=1; i<N-2; i++){
        // dynamics of orientation - abstrute change
        fg[0] += W_DELTA_DYN * CppAD::pow(vars[delta_start + i + 1] - vars[delta_start + i], 2);
        // dynamics of acceleration - abstrute change
        fg[0] += CppAD::pow(vars[a_start + i + 1] - vars[a_start + i], 2);
    }


    // ******************** Part II: Define the Constraints ******************//
    ...
    for (size_t t = 1; t < N; ++t) {
      ...
      // Need to update - f0 is not right
      ...
      fg[1 + psi_start + t] = psi1 - (psi0 + v0 * delta0 * (dt + latency) / Lf);
      fg[1 + v_start + t] = v1 - (v0 + a0 * (dt + latency));
      fg[1 + cte_start + t] = cte1 - ((f0 - y0) + (v0 * CppAD::sin(psi0) * (dt + latency)));
...
std::vector<double> MPC::Solve(const VectorXd &state, const VectorXd &coeffs) {
  ...
  // ****************** Part V: Save and Return Results ******************//
  this->pred_x = {};
  this->pred_y = {};
  for (size_t i = 0; i < N; i++) {
    this->pred_x.push_back(solution.x[x_start + i]);
    this->pred_y.push_back(solution.x[y_start + i]);
  }
